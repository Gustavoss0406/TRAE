...
import { oddsProcedure, predictionsProcedure } from "./odds-predictions-router";
import {
  getCountries,
  getTimezones,
  getLeagues,
...
const standingsProcedure = publicProcedure
  ...
  .query(async ({ input }) => {
    ...
    // Extract standings with team info
    const standingsForNormalizer = standingsData.map((row: any) => ({
      ...row.standing,
      team: row.team,
    }));
